...
static HELPCONTEXT g_hc = {
	"html/vessels/Scout.chm",
	0,
	"html/vessels/Scout.chm::/Scout.hhc",
	"html/vessels/Scout.chm::/Scout.hhk"
};

SURFHANDLE Scout::panel2dtex = NULL;

// ==============================================================
// Local prototypes

BOOL CALLBACK Ctrl_DlgProc (HWND, UINT, WPARAM, LPARAM);
void UpdateCtrlDialog (Scout *dg, HWND hWnd = 0);

//BOOL CALLBACK Damage_DlgProc (HWND, UINT, WPARAM, LPARAM);
//void UpdateDamageDialog (Scout *dg, HWND hWnd = 0);
...
// ==============================================================
// Specialised vessel class Scout
// ==============================================================

// --------------------------------------------------------------
// Constructor
// --------------------------------------------------------------
Scout::Scout (OBJHANDLE hObj, int fmodel)
: VESSEL3 (hObj, fmodel)
{
	int i, j;
...
// --------------------------------------------------------------
// Overloaded clbkPreStep
//---------------------------------------------------------------
void Scout::clbkPreStep (double simt, double simdt, double mjd)
{
VECTOR3 gforce, unit_grav, vector_empuxo;
	...
	//double f=gravity*GetMass()*9.8;
	double g=gravity/GetMass();
	double bouyancy = GetAtmDensity () * 1400 * g;
	unit_grav=gforce/gravity; //unit vector
	double level = GetThrusterLevel (th_hover[0]);
	vector_empuxo=(gforce*-1*level*.99); // thrust being always opposite to gforce
	AddForce(vector_empuxo + _V(0, bouyancy, 0),_V(0,0,0));

	SetThrusterMax0 (th_main [0], GetMass() * 9.8);
	SetThrusterMax0 (th_main [1], GetMass() * 9.8);
	double RCS_MOD = .01;
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_PITCHUP, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_PITCHUP, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_PITCHDOWN, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_PITCHDOWN, 1), GetMass() * 9.8 * RCS_MOD / 2);
    SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_UP, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_UP, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_DOWN, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_DOWN, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_YAWLEFT, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_YAWLEFT, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_YAWRIGHT, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_YAWRIGHT, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_LEFT, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_LEFT, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_RIGHT, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_RIGHT, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_BANKLEFT, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_BANKLEFT, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_BANKRIGHT, 0), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_BANKRIGHT, 1), GetMass() * 9.8 * RCS_MOD / 2);
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_FORWARD, 0), GetMass() * 9.8 * RCS_MOD );
	SetThrusterMax0 (GetGroupThruster(THGROUP_ATT_BACK, 0), GetMass() * 9.8 * RCS_MOD );
...
// --------------------------------------------------------------
void Scout::CreatePanelElements ()
{
	int i, j;
...
// --------------------------------------------------------------
void Scout::DefineAnimations ()
{
	// ***** Landing gear animation *****
	
	ANIMATIONCOMPONENT_HANDLE rParent01, rParent02, rParent03, rParent04;
	ANIMATIONCOMPONENT_HANDLE lParent01, lParent02, lParent03, lParent04;
	ANIMATIONCOMPONENT_HANDLE cParent01, cParent02, cParent03, cParent04;

	static UINT RCover01Grp[1] = {MESHGRP_RCOVER01};
	static MGROUP_TRANSLATE RCoverT01 (0, RCover01Grp, 1,
		_V(-.02755,.09266,-.00918));
	static MGROUP_TRANSLATE RCoverT02 (0, RCover01Grp, 1,
		_V(1.5,.51572,-.05621));
	static UINT RCover02Grp[1] = {MESHGRP_RCOVER02};
	static MGROUP_TRANSLATE RCoverT03 (0, RCover02Grp, 1,
		_V(-.02756,.09267,-.00906));
	static MGROUP_TRANSLATE RCoverT04 (0, RCover02Grp, 1,
		_V(-1.49834,-.44113,.04512));
	static UINT LCover01Grp[1] = {MESHGRP_LCOVER01};
	static MGROUP_TRANSLATE LCoverT01 (0, LCover01Grp, 1,
		_V(.02761,.09265,-.00913));
	static MGROUP_TRANSLATE LCoverT02 (0, LCover01Grp, 1,
		_V(-1.55728,.45979,-.04374));
	static UINT LCover02Grp[1] = {MESHGRP_LCOVER02};
	static MGROUP_TRANSLATE LCoverT03 (0, LCover02Grp, 1,
		_V(.02757,.09267,-.00905));
	static MGROUP_TRANSLATE LCoverT04 (0, LCover02Grp, 1,
		_V(1.50935,-.44528,.03827));
	static UINT CCoverLGrp[1] = {MESHGRP_CCOVERL};
	static MGROUP_TRANSLATE CCoverLT01 (0, CCoverLGrp, 1,
		_V(.03365,.09071,-.00829));
	static MGROUP_TRANSLATE CCoverLT02 (0, CCoverLGrp, 1,
		_V(-1.60194,.5904,-.04214));
	static UINT CCoverRGrp[1] = {MESHGRP_CCOVERR};
	static MGROUP_TRANSLATE CCoverRT01 (0, CCoverRGrp, 1,
		_V(-.03365,.09071,-.00829));
	static MGROUP_TRANSLATE CCoverRT02 (0, CCoverRGrp, 1,
		_V(1.63734,.60224,-.05632));

	static UINT RNull01Grp[1] = {MESHGRP_RNULL01};
	static MGROUP_TRANSLATE RNull01T01 (0, RNull01Grp, 1,
		_V(0,-1.03015,0));
	static UINT RGear01Grp[1] = {MESHGRP_RGEAR01};
	RGear01R01 = new MGROUP_ROTATE (0, RGear01Grp, 1,
		_V(4.85476,.06448,-2.62521), _V(0,0,1), (float)(-68.603*RAD));
	static UINT RGear02Grp[1] = {MESHGRP_RGEAR02};
	RGear02T01 = new MGROUP_TRANSLATE (0, RGear02Grp, 1,
		_V(1.04912,.2358,0));
	static UINT RNull02Grp[1] = {MESHGRP_RNULL02};
	RNull02R01 = new MGROUP_ROTATE (0, RNull02Grp, 1,
		_V(6.6736,.48238,-2.62521), _V(0,0,1), (float)(99.4*RAD));
	static UINT RFootGrp[1] = {MESHGRP_RFOOT};
	RFootR01 = new MGROUP_ROTATE (0, RFootGrp, 1,
		_V(6.6736,.48238,-2.62521), _V(0,0,1), (float)(-30.526*RAD));

	static UINT LNull01Grp[1] = {MESHGRP_LNULL01};
	static MGROUP_TRANSLATE LNull01T01 (0, LNull01Grp, 1,
		_V(0,-1.03015,0));
	static UINT LGear01Grp[1] = {MESHGRP_LGEAR01};
	LGear01R01 = new MGROUP_ROTATE (0, LGear01Grp, 1,
		_V(-4.85476,.06448,-2.62521), _V(0,0,1), (float)(68.603*RAD));
	static UINT LGear02Grp[1] = {MESHGRP_LGEAR02};
	LGear02T01 = new MGROUP_TRANSLATE (0, LGear02Grp, 1,
		_V(-1.04912,.2358,0));
	static UINT LNull02Grp[1] = {MESHGRP_LNULL02};
	LNull02R01 = new MGROUP_ROTATE (0, LNull02Grp, 1,
		_V(-6.6736,.48238,-2.62521), _V(0,0,1), (float)(-99.4*RAD));
	static UINT LFootGrp[1] = {MESHGRP_LFOOT};
	LFootR01 = new MGROUP_ROTATE (0, LFootGrp, 1,
		_V(-6.6736,.48238,-2.62521), _V(0,0,1), (float)(30.526*RAD));

	static UINT CNull01Grp[1] = {MESHGRP_CNULL01};
	static MGROUP_TRANSLATE CNull01T01(0, CNull01Grp, 1,
		_V(0,-.67742,.64967));
	static UINT CGear01Grp[1] = {MESHGRP_CGEAR01};
	CGear01R01 = new MGROUP_ROTATE (0, CGear01Grp, 1,
		_V(0,-.24638,10.91034), _V(1,0,0), (float)(68.581*RAD));
	static UINT CGear02Grp[1] = {MESHGRP_CGEAR02};
	CGear02T01 = new MGROUP_TRANSLATE (0, CGear02Grp, 1,
		_V(0,.21831,1.05288));
	static UINT CNull02Grp[1] = {MESHGRP_CNULL02};
	CNull02R01 = new MGROUP_ROTATE (0, CNull02Grp, 1,
		_V(0,.11796,12.73709), _V(1,0,0), (float)(-87.488*RAD));
	static UINT CFootGrp[1] = {MESHGRP_CFOOT};
	CFootR01 = new MGROUP_ROTATE (0, CFootGrp, 1,
		_V(0,.11796,12.73709), _V(1,0,0), (float)(22.241*RAD));

	//RFootR03 = new MGROUP_ROTATE (0, RFootGrp, 1,
	//	_V(6.67523,.045916,-2.62521), _V(0,0,1), (float)(32.151*RAD));
	...
	//	_V( 2.175,-1.178,-3.438), _V(0,0,1), (float)( 90*RAD));
	anim_gear01 = CreateAnimation (0);
	AddAnimationComponent (anim_gear01, 0, .1, &RCoverT01);
	AddAnimationComponent (anim_gear01, .1, .3, &RCoverT02);
	AddAnimationComponent (anim_gear01, 0, .1, &RCoverT03);
	AddAnimationComponent (anim_gear01, .1, .3, &RCoverT04);
	AddAnimationComponent (anim_gear01, 0, .1, &LCoverT01);
	AddAnimationComponent (anim_gear01, .1, .3, &LCoverT02);
	AddAnimationComponent (anim_gear01, 0, .1, &LCoverT03);
	AddAnimationComponent (anim_gear01, .1, .3, &LCoverT04);
	AddAnimationComponent (anim_gear01, 0, .1, &CCoverLT01);
	AddAnimationComponent (anim_gear01, .1, .3, &CCoverLT02);
	AddAnimationComponent (anim_gear01, 0, .1, &CCoverRT01);
	AddAnimationComponent (anim_gear01, .1, .3, &CCoverRT02);

	rParent01 = AddAnimationComponent (anim_gear01, .2, .8, &RNull01T01);
	rParent02 = AddAnimationComponent (anim_gear01, .2, .8, RGear01R01, rParent01);
	rParent03 = AddAnimationComponent (anim_gear01, .6, 1.0, RGear02T01, rParent02);
	rParent04 = AddAnimationComponent (anim_gear01, .2, .75, RNull02R01, rParent03);
	AddAnimationComponent (anim_gear01, .75, 1.0, RFootR01, rParent04);

	lParent01 = AddAnimationComponent (anim_gear01, .2, .8, &LNull01T01);
	lParent02 = AddAnimationComponent (anim_gear01, .2, .8, LGear01R01, lParent01);
	lParent03 = AddAnimationComponent (anim_gear01, .6, 1.0, LGear02T01, lParent02);
	lParent04 = AddAnimationComponent (anim_gear01, .2, .75, LNull02R01, lParent03);
	AddAnimationComponent (anim_gear01, .75, 1.0, LFootR01, lParent04);

	cParent01 = AddAnimationComponent (anim_gear01, .2, .8, &CNull01T01);
	cParent02 = AddAnimationComponent (anim_gear01, .2, .6, CGear01R01, cParent01);
	cParent03 = AddAnimationComponent (anim_gear01, .6, 1.0, CGear02T01, cParent02);
	cParent04 = AddAnimationComponent (anim_gear01, .2, .75, CNull02R01, cParent03);
	AddAnimationComponent (anim_gear01, .75, 1.0, CFootR01, cParent04);

	//AddAnimationComponent (anim_gear01, .8, .9, RFootR02, parent03);
	//AddAnimationComponent (anim_gear01, .9, 1, RFootR03, parent03);
...
// --------------------------------------------------------------
void Scout::PaintMarkings (SURFHANDLE tex)
{
...
// --------------------------------------------------------------
void Scout::InitPanel (int panel)
{
	DWORD i;
...
bool Scout::clbkDrawHUD (int mode, const HUDPAINTSPEC *hps, oapi::Sketchpad *skp)
{
...
void Scout::clbkRenderHUD (int mode, const HUDPAINTSPEC *hps, SURFHANDLE hTex)
{
...
void Scout::ActivateLandingGear (DoorStatus action)
{
	...
	if (action <= DOOR_OPEN) {
		...
		SetAnimation (anim_gear01, gear_proc);
		//SetAnimation (anim_gear02, gear_proc);
...
void Scout::SetGearParameters (double state)
{
	if (state == 1.0) {
		SetTouchdownPoints (_V(0,-4.31046,13.13516), _V(-6.47659,-4.31046,-2.67023), _V(6.47659,-4.31046,-2.67023));
		SetSurfaceFrictionCoeff (0.4, 0.4);
		SetNosewheelSteering (false);
	} else {
		SetTouchdownPoints (_V(0,-1.14307,16.49258), _V(-5.37602,-4.44096,-1.98786), _V(7.95691,-.46964,-2.12387));
		SetSurfaceFrictionCoeff (0.4, 0.4);
		SetNosewheelSteering (false);
	}
}

void Scout::ActivateRCover (DoorStatus action)
{
	bool close = (action == DOOR_CLOSED || action == DOOR_CLOSING);
...
void Scout::ActivateDockingPort (DoorStatus action)
{
	bool close = (action == DOOR_CLOSED || action == DOOR_CLOSING);
	nose_status = action;
	if (action <= DOOR_OPEN) {
...
void Scout::RevertDockingPort ()
{
	ActivateDockingPort (nose_status == DOOR_CLOSED || nose_status == DOOR_CLOSING ?
						 DOOR_OPENING : DOOR_CLOSING);
}

void Scout::ActivateHatch (DoorStatus action)
{
	bool close = (action == DOOR_CLOSED || action == DOOR_CLOSING);
...
void Scout::RevertHatch ()
{
	ActivateHatch (hatch_status == DOOR_CLOSED || hatch_status == DOOR_CLOSING ?
				   DOOR_OPENING : DOOR_CLOSING);
}

void Scout::ActivateLadder (DoorStatus action)
{
	bool close = (action == DOOR_CLOSED || action == DOOR_CLOSING);
...
void Scout::RevertLadder ()
{
	ActivateLadder (ladder_status == DOOR_CLOSED || ladder_status == DOOR_CLOSING ?
					DOOR_OPENING : DOOR_CLOSING);
}

void Scout::ActivateOuterAirlock (DoorStatus action)
{
	bool close = (action == DOOR_CLOSED || action == DOOR_CLOSING);
	olock_status = action;
	if (action <= DOOR_OPEN) {
...
void Scout::RevertOuterAirlock ()
{
	ActivateOuterAirlock (olock_status == DOOR_CLOSED || olock_status == DOOR_CLOSING ?
		                  DOOR_OPENING : DOOR_CLOSING);
}

void Scout::ActivateInnerAirlock (DoorStatus action)
{
	bool close = (action == DOOR_CLOSED || action == DOOR_CLOSING);
	ilock_status = action;
	if (action <= DOOR_OPEN) {
...
void Scout::RevertInnerAirlock ()
{
	ActivateInnerAirlock (ilock_status == DOOR_CLOSED || ilock_status == DOOR_CLOSING ?
		                  DOOR_OPENING : DOOR_CLOSING);
}

void Scout::ActivateAirbrake (DoorStatus action)
{
	brake_status = action;
...
void Scout::RevertAirbrake (void)
{
	ActivateAirbrake (brake_status == DOOR_CLOSED || brake_status == DOOR_CLOSING ?
		DOOR_OPENING : DOOR_CLOSING);
}

void Scout::ActivateRadiator (DoorStatus action)
{
	bool close = (action == DOOR_CLOSED || action == DOOR_CLOSING);
...
void Scout::SetDockingLight (bool on)
{
	beacon[7].active = on;
...
void Scout::SetMainRetroLevel (int which, double lmain, double lretro)
{
...
void Scout::SetScramLevel (int which, double level)
{
	for (int i = 0; i < 2; i++) {
		if (which != 1-i) {
...
void Scout::EnableRetroThrusters (bool state)
{
	for (int i = 0; i < 2; i++)
		SetThrusterResource (th_retro[i], state ? ph_main : NULL);
}

bool Scout::ShiftHoverBalance (int mode)
{
	hbswitch = mode;
...
bool Scout::IncMainPGimbal (int which, int mode)
{
	VECTOR3 dir;
	int i, idx;
	bool redraw = false;
...
void Scout::AdjustMainPGimbal (int &mode)
{
	VECTOR3 dir;
	int i, idx;
...
bool Scout::IncMainYGimbal (int which, int mode)
{
	VECTOR3 dir;
	int i, idx;
	bool redraw = false;
...
void Scout::AdjustMainYGimbal (int &mode)
{
	VECTOR3 dir;
	int i, idx;
...
bool Scout::IncScramGimbal (int which, int mode)
{
	VECTOR3 dir;
	int i, idx;
	bool redraw = false;
...
void Scout::AdjustScramGimbal (int &mode)
{
	VECTOR3 dir;
	int i, idx;
...
void Scout::AdjustHoverBalance (int &mode)
{
	if (mode == 1) {
		double t0 = GetThrusterMax0 (th_hover[0]);
		double t1 = GetThrusterMax0 (th_hover[1]);
...
bool Scout::GetBeaconState (int which)
{
	switch (which) {
...
bool Scout::RedrawPanel_AOA (SURFHANDLE surf, bool force)
{
...
bool Scout::RedrawPanel_Slip (SURFHANDLE surf, bool force)
{
	static const double dial_max = RAD*165.0;
	static const double eps = 1e-2;
...
bool Scout::RedrawPanel_Wingload (SURFHANDLE surf, bool force)
{
...
bool Scout::RedrawPanel_ScramTSFC (SURFHANDLE surf)
{
	bool redraw = false;
	for (int i = 0; i < 2; i++) {
...
bool Scout::RedrawPanel_ScramFlow (SURFHANDLE surf)
{
	bool redraw = false;
	for (int i = 0; i < 2; i++) {
...
bool Scout::RedrawPanel_MainProp (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_MainPropMass (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_RCSProp (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_RCSPropMass (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_ScramProp (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_ScramPropMass (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_RotVelocity (SURFHANDLE surf, int which)
{
	int idx;
	double v, av;
	VECTOR3 vrot;
...
bool Scout::RedrawPanel_RotAcceleration (SURFHANDLE surf, int which)
{
	int idx;
	double a, aa;
	VECTOR3 arot;
...
bool Scout::RedrawPanel_RotMoment (SURFHANDLE surf, int which)
{
	int idx;
	double m, am;
	VECTOR3 amom;
...
bool Scout::RedrawPanel_ScramTempDisp (SURFHANDLE surf)
{
	double T, phi;
...
bool Scout::RedrawPanel_WBrake (SURFHANDLE surf, int which)
{
...
void Scout::RedrawPanel_MFDButton (SURFHANDLE surf, int mfd, int side)
{
	HDC hDC = oapiGetDC (surf);
...
bool Scout::RedrawPanel_IndicatorPair (SURFHANDLE surf, int *p, int range)
{
...
bool Scout::RedrawPanel_Number (SURFHANDLE surf, int x, int y, char *num)
{
	for (; *num; num++) {
...
bool Scout::RedrawPanel_MainFlow (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_RetroFlow (SURFHANDLE surf)
{
    double gaugeSize = 66.99;  // pointer can move 66 pixels; also round up to next pixel
	bool redraw = false;
	for (int i = 0; i < 2; i++) {
...
bool Scout::RedrawPanel_HoverFlow (SURFHANDLE surf)
{
    double gaugeSize = 66.99;  // pointer can move 66 pixels; also round up to next pixel
...
double Scout::GetThrusterFlowRate(THRUSTER_HANDLE th)
{
...
bool Scout::RedrawPanel_MainTSFC (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_GimbalScramDisp (SURFHANDLE surf)
{
...
bool Scout::RedrawPanel_HoverBalanceDisp (SURFHANDLE surf)
{
	oapiBlt (surf, srf[8], 0, hbalanceidx, 0, 0, 6, 7, SURF_PREDEF_CK);
	return true;
}

bool Scout::RedrawPanel_GearIndicator (SURFHANDLE surf)
{
	switch (gear_status) {
...
bool Scout::RedrawPanel_NoseconeIndicator (SURFHANDLE surf)
{
	switch (nose_status) {
...
void Scout::UpdateStatusIndicators ()
{
	if (!vcmesh) return;
...
void Scout::SetPassengerVisuals ()
{
...
void Scout::DrawNeedle (HDC hDC, int x, int y, double rad, double angle, double *pangle, double vdial)
{
...
void Scout::SetVC_ScramGimbalMode ()
{
	if (!vcmesh) return;
...
void Scout::SetVC_HoverBalanceMode ()
{
	if (!vcmesh) return;
...
// --------------------------------------------------------------
void Scout::clbkSetClassCaps (FILEHANDLE cfg)
{
	// *************** physical parameters **********************
	...
	// main thrusters
	th_main[0] = CreateThruster (_V(-3.419460,0,-8.352194), _V(0,0,1), 242471, ph_main, 4e50, 4e50);
	th_main[1] = CreateThruster (_V( 3.419460,0,-8.352194), _V(0,0,1), 242471, ph_main, 4e50, 4e50);
	...
	//AddExhaustStream (th_main[0], _V(-3.419460,0,-8.352194), &exhaust_main);
	//AddExhaustStream (th_main[1], _V( 3.419460,0,-8.352194), &exhaust_main);
	...
	th_hover[0] = CreateThruster (_V(0,0,3), _V(0,1,0), 0, ph_main, ISP, ISP*ispscale);
	th_hover[1] = CreateThruster (_V(0,0,-3), _V(0,1,0), 0, ph_main, ISP, ISP*ispscale);
	...
	AddExhaust (th_att_rot[0], 0.6 * eScale,  0.078 * eScale, _V(-0.75, -.31154,  22.84125), _V(0,-1,0));
	AddExhaust (th_att_rot[0], 0.6 * eScale,  0.078 * eScale, _V( 0.75, -.31154,  22.84125), _V(0,-1,0));
	AddExhaust (th_att_rot[1], 0.79 * eScale, 0.103 * eScale, _V(-0.75, 3.38142,  -7.91447), _V(0, 1,0));
	AddExhaust (th_att_rot[1], 0.79 * eScale, 0.103 * eScale, _V( 0.75, 3.38142,  -7.91447), _V(0, 1,0));
	AddExhaust (th_att_rot[2], 0.6 * eScale,  0.078 * eScale, _V(-0.75, .31154,  22.84125), _V(0, 1,0));
	AddExhaust (th_att_rot[2], 0.6 * eScale,  0.078 * eScale, _V( 0.75, .31154,  22.84125), _V(0, 1,0));
	AddExhaust (th_att_rot[3], 0.79 * eScale, 0.103 * eScale, _V(-0.75,-3.38142,  -7.91447), _V(0,-1,0));
	AddExhaust (th_att_rot[3], 0.79 * eScale, 0.103 * eScale, _V( 0.75,-3.38142,  -7.91447), _V(0,-1,0));
	...
	AddExhaust (th_att_rot[0], 0.6 * eScale,  0.156 * eScale, _V(  1.26052, 0.0, 22.84125), _V(1,0,0));
	AddExhaust (th_att_rot[1], 0.94 * eScale, 0.122 * eScale, _V(-11.19383, 0.0, -7.01367), _V(-1,0,0));
	AddExhaust (th_att_rot[2], 0.6 * eScale,  0.078 * eScale, _V( -1.26052, 0.0, 22.84125), _V(-1,0,0));
	AddExhaust (th_att_rot[3], 0.94 * eScale, 0.122 * eScale, _V( 11.19383, 0.0, -7.01367), _V(1,0,0));
	...
	AddExhaust (th_att_rot[0], 1.03 * eScale, 0.134 * eScale, _V(-8.57459, .23916, 0.0), _V(0, 1,0));
	AddExhaust (th_att_rot[1], 1.03 * eScale, 0.134 * eScale, _V( 8.57459,-.23916, 0.0), _V(0,-1,0));
	AddExhaust (th_att_rot[2], 1.03 * eScale, 0.134 * eScale, _V( 8.57459, .23916, 0.0), _V(0, 1,0));
	AddExhaust (th_att_rot[3], 1.03 * eScale, 0.134 * eScale, _V(-8.57459,-.23916, 0.0), _V(0,-1,0));
	...
	AddExhaust (th_att_lin[0], 0.6 * eScale, 0.078 * eScale, _V(0, -1.91251,-8.88969), _V(0,0,-1));
	AddExhaust (th_att_lin[0], 0.6 * eScale, 0.078 * eScale, _V(0, 1.91251,-8.88969), _V(0,0,-1));
	AddExhaust (th_att_lin[1], 0.6 * eScale, 0.078 * eScale, _V(0, .52496, 22.84125), _V(0,0,1));
	AddExhaust (th_att_lin[1], 0.6 * eScale, 0.078 * eScale, _V(0, -.52496, 22.84125), _V(0,0,1));
	...
	hwing = CreateAirfoil3 (LIFT_VERTICAL, _V(0,0,-.3), VLiftCoeff, 0, 5, 90, 1.5);
	...
	CreateControlSurface3 (AIRCTRL_ELEVATOR,     1.4, 1.5, _V(   0,0,-7.2), AIRCTRL_AXIS_XPOS, 1.0);
	CreateControlSurface3 (AIRCTRL_RUDDER,       0.8, 1.5, _V(   0,0,-7.2), AIRCTRL_AXIS_YPOS, 1.0);
	hlaileron = CreateControlSurface3 (AIRCTRL_AILERON, 0.3, 1.5, _V( 7.5,0,-7.2), AIRCTRL_AXIS_XPOS, 1.0);
	hraileron = CreateControlSurface3 (AIRCTRL_AILERON, 0.3, 1.5, _V(-7.5,0,-7.2), AIRCTRL_AXIS_XNEG, 1.0);
	CreateControlSurface3 (AIRCTRL_ELEVATORTRIM, 0.3, 1.5, _V(   0,0,-7.2), AIRCTRL_AXIS_XPOS, 1.0);
	...
	SetMeshVisibilityMode (AddMesh (exmesh_tpl = oapiLoadMeshGlobal (ScramVersion() ? "scout" : "scout")), MESHVIS_EXTERNAL);
...
// --------------------------------------------------------------
void Scout::clbkLoadStateEx (FILEHANDLE scn, void *vs)
{
    char *line;
...
// --------------------------------------------------------------
void Scout::clbkSaveState (FILEHANDLE scn)
{
	char cbuf[256];
	int i;
...
// --------------------------------------------------------------
bool Scout::clbkPlaybackEvent (double simt, double event_t, const char *event_type, const char *event)
{
...
// Create DG visual
// --------------------------------------------------------------
void Scout::clbkVisualCreated (VISHANDLE vis, int refcount)
{
	visual = vis;
...
// Destroy DG visual
// --------------------------------------------------------------
void Scout::clbkVisualDestroyed (VISHANDLE vis, int refcount)
{
	visual = NULL;
	exmesh = NULL;
	vcmesh = NULL;
...
// --------------------------------------------------------------
void Scout::clbkMFDMode (int mfd, int mode)
{
	switch (mfd) {
	case MFD_LEFT:
...
// --------------------------------------------------------------
void Scout::clbkRCSMode (int mode)
{
...
// --------------------------------------------------------------
void Scout::clbkADCtrlMode (DWORD mode)
{
...
// --------------------------------------------------------------
void Scout::clbkHUDMode (int mode)
{
	oapiTriggerRedrawArea (0, 0, AID_HUDMODE);
}
...
// --------------------------------------------------------------
void Scout::clbkNavMode (int mode, bool active)
{
...
// Frame update
// --------------------------------------------------------------
void Scout::clbkPostStep (double simt, double simdt, double mjd)
{
	...
	if (rcover_status >= DOOR_CLOSING) {
		...
		//SetAnimation (anim_gear, rcover_proc);
		UpdateStatusIndicators();
	}
...
bool Scout::clbkLoadGenericCockpit ()
{
...
// --------------------------------------------------------------
#ifdef UNDEF
bool Scout::clbkLoadPanel (int id)
{
	int xofs;
	ReleaseSurfaces();
...
bool Scout::clbkLoadPanel2D (int id, PANELHANDLE hPanel, DWORD viewW, DWORD viewH)
{
	switch (id) {
	case 0:
...
void Scout::SetPanelScale (PANELHANDLE hPanel, DWORD viewW, DWORD viewH)
{
...
void Scout::DefinePanelMain (PANELHANDLE hPanel)
{
	MESHGROUP grp;
...
void Scout::DefinePanelOverhead (PANELHANDLE hPanel)
{
	DWORD i;
...
// --------------------------------------------------------------
bool Scout::clbkPanelMouseEvent (int id, int event, int mx, int my, void *context)
{
	if (context) {
...
// --------------------------------------------------------------
bool Scout::clbkPanelRedrawEvent (int id, int event, SURFHANDLE surf, void *context)
{
	if (context) {
		PanelElement *pe = (PanelElement*)context;
...
// --------------------------------------------------------------
bool Scout::clbkVCMouseEvent (int id, int event, VECTOR3 &p)
{
...
// --------------------------------------------------------------
bool Scout::clbkVCRedrawEvent (int id, int event, SURFHANDLE surf)
{
	switch (id) {
	case AID_MFD1_LBUTTONS:
...
// --------------------------------------------------------------
int Scout::clbkConsumeDirectKey (char *kstate)
{
...
// --------------------------------------------------------------
int Scout::clbkConsumeBufferedKey (DWORD key, bool down, char *kstate)
{
...
// --------------------------------------------------------------
int Scout::clbkGeneric (int msgid, int prm, void *context)
{
	switch (msgid) {
...
DLLCLBK void InitModule (HINSTANCE hModule)
{
	...
	// allocate textures
	Scout::panel2dtex = oapiLoadTexture ("Scout\\dg_panel.dds");
}
...
DLLCLBK void ExitModule (HINSTANCE hModule)
{
	...
	// deallocate textures
	oapiDestroySurface (Scout::panel2dtex);
}
...
DLLCLBK VESSEL *ovcInit (OBJHANDLE hvessel, int flightmodel)
{
	...
	return new Scout (hvessel, flightmodel);
}
...
Scout *GetDG (HWND hDlg)
{
	// retrieve DG interface from scenario editor
	OBJHANDLE hVessel;
	SendMessage (hDlg, WM_SCNEDITOR, SE_GETVESSEL, (LPARAM)&hVessel);
	return (Scout*)oapiGetVesselInterface (hVessel);
}

void UpdateDamage (HWND hTab, Scout *dg)
{
	int i;
	char cbuf[256];
...
BOOL CALLBACK EdPg1Proc (HWND hTab, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDHELP:
			...
			return TRUE;
		case IDC_GEAR_UP:
			GetDG(hTab)->ActivateLandingGear (Scout::DOOR_CLOSED);
			return TRUE;
		case IDC_GEAR_DOWN:
			GetDG(hTab)->ActivateLandingGear (Scout::DOOR_OPEN);
			return TRUE;
		case IDC_RETRO_CLOSE:
			GetDG(hTab)->ActivateRCover (Scout::DOOR_CLOSED);
			return TRUE;
		case IDC_RETRO_OPEN:
			GetDG(hTab)->ActivateRCover (Scout::DOOR_OPEN);
			return TRUE;
		case IDC_OLOCK_CLOSE:
			GetDG(hTab)->ActivateOuterAirlock (Scout::DOOR_CLOSED);
			return TRUE;
		case IDC_OLOCK_OPEN:
			GetDG(hTab)->ActivateOuterAirlock (Scout::DOOR_OPEN);
			return TRUE;
		case IDC_ILOCK_CLOSE:
			GetDG(hTab)->ActivateInnerAirlock (Scout::DOOR_CLOSED);
			return TRUE;
		case IDC_ILOCK_OPEN:
			GetDG(hTab)->ActivateInnerAirlock (Scout::DOOR_OPEN);
			return TRUE;
		case IDC_NCONE_CLOSE:
			GetDG(hTab)->ActivateDockingPort (Scout::DOOR_CLOSED);
			return TRUE;
		case IDC_NCONE_OPEN:
			GetDG(hTab)->ActivateDockingPort (Scout::DOOR_OPEN);
			return TRUE;
		case IDC_LADDER_RETRACT:
			GetDG(hTab)->ActivateLadder (Scout::DOOR_CLOSED);
			return TRUE;
		case IDC_LADDER_EXTEND:
			GetDG(hTab)->ActivateLadder (Scout::DOOR_OPEN);
			return TRUE;
		case IDC_HATCH_CLOSE:
			GetDG(hTab)->ActivateHatch (Scout::DOOR_CLOSED);
			return TRUE;
		case IDC_HATCH_OPEN:
			GetDG(hTab)->ActivateHatch (Scout::DOOR_OPEN);
			return TRUE;
		case IDC_RADIATOR_RETRACT:
			GetDG(hTab)->ActivateRadiator (Scout::DOOR_CLOSED);
			return TRUE;
		case IDC_RADIATOR_EXTEND:
			GetDG(hTab)->ActivateRadiator (Scout::DOOR_OPEN);
			return TRUE;
		}
		break;
	}
...
BOOL CALLBACK EdPg2Proc (HWND hTab, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		char cbuf[256];
		dg = (Scout*)oapiGetVesselInterface ((OBJHANDLE)lParam);
		for (i = 0; i < 4; i++)
...
{
	Scout *dg = (Scout*)oapiGetVesselInterface (hVessel);
...
{
	Scout *dg = (uMsg == WM_INITDIALOG ? (Scout*)lParam : (Scout*)oapiGetDialogContext (hWnd));
	...
	case WM_COMMAND:
		...
		case IDCANCEL:
			...
			return TRUE;
		case IDC_GEAR_UP:
			dg->ActivateLandingGear (Scout::DOOR_CLOSING);
			return 0;
		case IDC_GEAR_DOWN:
			dg->ActivateLandingGear (Scout::DOOR_OPENING);
			return 0;
		case IDC_RETRO_CLOSE:
			dg->ActivateRCover (Scout::DOOR_CLOSING);
			return 0;
		case IDC_RETRO_OPEN:
			dg->ActivateRCover (Scout::DOOR_OPENING);
			return 0;
		case IDC_NCONE_CLOSE:
			dg->ActivateDockingPort (Scout::DOOR_CLOSING);
			return 0;
		case IDC_NCONE_OPEN:
			dg->ActivateDockingPort (Scout::DOOR_OPENING);
			return 0;
		case IDC_OLOCK_CLOSE:
			dg->ActivateOuterAirlock (Scout::DOOR_CLOSING);
			return 0;
		case IDC_OLOCK_OPEN:
			dg->ActivateOuterAirlock (Scout::DOOR_OPENING);
			return 0;
		case IDC_ILOCK_CLOSE:
			dg->ActivateInnerAirlock (Scout::DOOR_CLOSING);
			return 0;
		case IDC_ILOCK_OPEN:
			dg->ActivateInnerAirlock (Scout::DOOR_OPENING);
			return 0;
		case IDC_LADDER_RETRACT:
			dg->ActivateLadder (Scout::DOOR_CLOSING);
			return 0;
		case IDC_LADDER_EXTEND:
			dg->ActivateLadder (Scout::DOOR_OPENING);
			return 0;
		case IDC_HATCH_CLOSE:
			dg->ActivateHatch (Scout::DOOR_CLOSING);
			return 0;
		case IDC_HATCH_OPEN:
			dg->ActivateHatch (Scout::DOOR_OPENING);
			return 0;
		case IDC_RADIATOR_RETRACT:
			dg->ActivateRadiator (Scout::DOOR_CLOSING);
			return 0;
		case IDC_RADIATOR_EXTEND:
			dg->ActivateRadiator (Scout::DOOR_OPENING);
			return 0;
		case IDC_NAVLIGHT:
...
void UpdateCtrlDialog (Scout *dg, HWND hWnd)
{
...
void UpdateDamageDialog (Scout *dg, HWND hWnd)
{
